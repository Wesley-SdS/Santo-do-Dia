  ...
  if (donations.length === 0) {
    return (
      <div className="mt-4 rounded-xl border border-dashed border-border p-8 text-center">
        <p className="text-sm text-muted-foreground">Seja o primeiro a apoiar o SantoDia!</p>
      </div>
    );
  }
  ...
  return (
    <div className="mt-4 space-y-2">
      {donations.map((donation) => (
        <div key={donation.id} className="flex items-center justify-between rounded-lg bg-card p-3">
          <div className="flex items-center gap-3">
            ...
            <span className="text-sm text-foreground">{donation.displayName ?? 'Anônimo'}</span>
          </div>
